				...
				<div className="row">
					{this.props.albums.length !== 0 && (
						<div className="main-info" id="info">
							Results found for "
							<span id="artist-name">{this.props.lastSearch}</span>"
					...
					{this.props.albums.length === 0 && (
						<div className="main-info" id="no-results">
							...
							<div id="no-results-secondary">
								Check your internet connection and try looking for an
								artist in the search bar
							</div>
						</div>
					)}
				</div>
				<div className="row" id="albums-container">
					...
					>
						{this.props.albums.map(album => {
							...
							return <Album data={album} key={albumId} />;
						})}
					</ReactCSSTransitionGroup>
				</div>
			</div>
